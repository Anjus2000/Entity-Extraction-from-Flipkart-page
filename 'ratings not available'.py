...
for a in soup.findAll('a',href=True, attrs={'class':'_1fQZEK'}):
    ...
    if rating:
    # if the element exists, extract the rating
        rating1 = rating.text
    else:
    # if the element does not exist, set the rating to "No rating available"
        rating1 = "No rating available"
    if rating1:
        ratings.append(rating1)
    else:
        ratings.append("No rating available")
    #ratings.append(rating1.get_text()) if rating else ''
